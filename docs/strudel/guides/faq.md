# FAQ
Source: https://strudel.cc/learn/faq/
Category: guides
...
- use the alternative SuperDirt audio engine. Read [this page](https://strudel.cc/learn/input-output/#oscsuperdirtstrudeldirt) to know more about it.
...
- Import folders [from the interface](https://strudel.cc/learn/samples/#from-disk-via-import-sounds-folder). These are stored locally in your web browser, and not uploaded.
...
- Host your sound library online on the web and [load them from an URL](https://strudel.cc/learn/samples/#loading-custom-samples)
...
You cannot, as there is no such place. For details regarding our stance towards AI/LLM, see [above](https://strudel.cc/learn/faq/#can-i-use-strudel-with-aillm-tools)

## How to run offline?

Strudel works offline just fine! There are multiple techniques for this, see [this explanation](https://strudel.cc/learn/pwa/#using-strudel-offline).
...
Strudel can send [MIDI and OSC](https://strudel.cc/learn/input-output/), which are protocols for communicating musical information.
...
See also [stack](https://strudel.cc/intro/#combining-patterns)
...
See also [hush](https://strudel.cc/learn/conditional-modifiers/#hush)
...
The `pick` method also works with jsons which have named elements, which makes it easier to read, see the [here](https://strudel.cc/learn/conditional-modifiers/#pick). `pickRestart` restarts the pattern upon picking it which can make a difference if the duration of the pick indexes doesn’t line up with the patterns which are picked - which is not the case here.
...
On the other hand, `n` is a way to reference the nth index of something. This something can be a scale (eg `n("0 2 4").scale("C:major")`) , but it can also be a particular note in a chord (see [https://strudel.cc/recipes/recipes/#arpeggios](https://strudel.cc/recipes/recipes/#arpeggios) for an example) .
