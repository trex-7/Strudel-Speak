...
| --- | --- | --- |
| rev | reverse | ```<br>n("0 2 4 6 ~ 7 9 5").scale("C:minor").rev()<br>``` |
| jux | split left/right, modify right | ```<br>n("0 2 4 6 ~ 7 9 5").scale("C:minor").jux(rev)<br>``` |
| add | add numbers / notes | ```<br>n("0 2 4 6 ~ 7 9 5".add("<0 1 2 1>")).scale("C:minor")<br>``` |
| ply | speed up each event n times | ```<br>s("bd sd [~ bd] sd").ply("<1 2 3>")<br>``` |
| off | copy, shift time & modify | ```<br>s("bd sd [~ bd] sd, hh*8").off(1/16, x=>x.speed(2))<br>``` |